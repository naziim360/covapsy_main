...
KP_VITESSE = 0.9  # Gain proportionnel
DISTANCE_CIBLE_M = 1  # Distance de sécurité cible (3m)
VITESSE_SECURITE_MIN = 0.5  # Vitesse min quand proche obstacle (m/s)
...
class WebotsGymEnvironment(Driver, gym.Env):
    ...
    def get_reward(self, obs):
        ...
        # ====================================================================
        SEUIL_COLLISION = 175.0 / LIDAR_RANGE_MM  # 175mm normalisé
        ...
        if mini < SEUIL_COLLISION:
            ...
            angle_change = abs(obs["previous_angle"][0] - self.consigne_angle / MAXANGLE_DEGRE)
            angle_penalty = 0.05 * angle_change
    ...
    def step(self, action):
        ...
        if self.observation is not None:
            front_sector = self.observation["current_lidar"][60:141]
...
def main():
    ...
    try:
        model = PPO.load("./model_refactored", env=env, device="cpu")
        print(" Modèle chargé")
    ...
    model.save("./model_refactored2")
    print(" Modèle sauvegardé : ./model_refactored2")
    """
